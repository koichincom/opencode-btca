...
async function runBtca(args: string[]): Promise<string> {
  const result = await Bun.$`btca ${args}`.quiet().nothrow();
...
export const config_model = tool({
  ...
  async execute({ provider, model }) {
    // btca config model doesn't exit after success, so we use timeout
    const result =
      await Bun.$`timeout 3 btca config model --provider ${provider} --model ${model}`
        .quiet()
        .nothrow();
    const stdout = result.stdout.toString().trim();
